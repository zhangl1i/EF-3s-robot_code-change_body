...
void HAL_TIM_PWM_MspInit(TIM_HandleTypeDef *htim)
{
    ...
    */
		...
		GPIO_Initure.Alternate=GPIO_AF1_TIM1;	//复用
...
//		PB0     ------> TIM3_CH3
		...
		GPIO_Initure.Pull=GPIO_NOPULL;         //上拉
		GPIO_Initure.Speed=GPIO_SPEED_FREQ_LOW;     //低速
		GPIO_Initure.Alternate=GPIO_AF2_TIM3;	//复用
...
//		PA7 		------>TIM3_CH2
		...
		GPIO_Initure2.Alternate=GPIO_AF2_TIM3;	
...
//        PB7     ------> TIM4_CH2 
		...
		GPIO_Initure.Pull=GPIO_NOPULL;         //上拉
		GPIO_Initure.Speed=GPIO_SPEED_FREQ_LOW;     //低速
		GPIO_Initure.Alternate=GPIO_AF2_TIM4;	//复用
		HAL_GPIO_Init(GPIOB,&GPIO_Initure);
		
//		    PD14     ------> TIM4_CH3
//        PD15     ------> TIM4_CH4 
		GPIO_Initure2.Pin=GPIO_PIN_14|GPIO_PIN_15;    
		GPIO_Initure2.Mode=GPIO_MODE_AF_PP;  	//复用推完输出
		GPIO_Initure2.Pull=GPIO_NOPULL;         //上拉
		GPIO_Initure2.Speed=GPIO_SPEED_FREQ_LOW;     //低速
		GPIO_Initure2.Alternate=GPIO_AF2_TIM4;	//复用
	...
	{	
		//msp
		GPIO_InitTypeDef GPIO_Initure;
		__HAL_RCC_TIM12_CLK_ENABLE();			//使能定时器
		__HAL_RCC_GPIOB_CLK_ENABLE();			//开启GPIOB时钟
//	    PB14     ------> TIM12_CH1
//	    PB15     ------> TIM12_CH2 
		...
		GPIO_Initure.Pull=GPIO_NOPULL;         //上拉
		GPIO_Initure.Speed=GPIO_SPEED_FREQ_LOW;     //低速
		GPIO_Initure.Alternate=GPIO_AF2_TIM12;	//复用
		HAL_GPIO_Init(GPIOB,&GPIO_Initure);
	...
	{	
		//msp
		GPIO_InitTypeDef GPIO_Initure;
		__HAL_RCC_TIM15_CLK_ENABLE();			//使能定时器
...
//	    PE6     ------> TIM15_CH2 
		...
		GPIO_Initure.Pull=GPIO_NOPULL;         //上拉
		GPIO_Initure.Speed=GPIO_SPEED_FREQ_LOW;     //低速
		GPIO_Initure.Alternate=GPIO_AF4_TIM15;	//复用为
		HAL_GPIO_Init(GPIOE,&GPIO_Initure);
	...
	{	
		//msp
		GPIO_InitTypeDef GPIO_Initure;
		__HAL_RCC_TIM16_CLK_ENABLE();			//使能定时器
		__HAL_RCC_GPIOB_CLK_ENABLE();			//开启GPIOB时钟
//	    PB8     ------> TIM16_CH1
		...
		GPIO_Initure.Pull=GPIO_NOPULL;         //上拉
		GPIO_Initure.Speed=GPIO_SPEED_FREQ_LOW;     //低速
		GPIO_Initure.Alternate=GPIO_AF1_TIM16;	
		HAL_GPIO_Init(GPIOB,&GPIO_Initure);
